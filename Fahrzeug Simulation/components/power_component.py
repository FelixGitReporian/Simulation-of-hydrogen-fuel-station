...
class Power_Component(Serializable, Simulatable):
    ...
    def __init__(self, timestep, input_link, input_link1, input_link2, input_link3, file_path = None):
        '''
        Parameters
        ----------
        ...
        self.specification = self.input_link1.specification
    ...
    def calculate(self):
        ...
        self.power_sum = self.input_link.power + self.input_link2.power_aircompressor + self.input_link3.power_pump

        # Calculate the Power output or input
        if self.specification == 'vehicle_fuelcell':
            if self.power_sum >= 0:  # generator(vehicle)
                self.__calculate_power_output()
            if self.power_sum < 0:  # motor(vehicle)
                self.__calculate_power_input()
        elif self.specification == 'vehicle_electric':
            if self.input_link.power >= 0:  # generator(vehicle)
                self.__calculate_power_output()
            if self.input_link.power < 0:  # motor(vehicle)
                self.__calculate_power_input()
    ...
    def __calculate_power_output (self):
        ...
        if self.specification == 'vehicle_fuelcell':
            if self.power_sum == 0:
                ...
                power_input = min(1, self.power_sum / self.power_nominal)
                self.efficiency = -((1 + self.voltage_loss_star) / (2 * self.resistance_loss_star * power_input)) \
                                  + (((1 + self.voltage_loss_star) ** 2 / (
                            2 * self.resistance_loss_star * power_input) ** 2) \
                                     + ((power_input - self.power_self_consumption_star) / (
                                    self.resistance_loss_star * power_input ** 2))) ** 0.5
    ...
    def __calculate_power_input (self):
        ...
        if self.specification == 'vehicle_fuelcell':
            ...
            power_output = (abs(self.power_sum) / self.power_nominal)
