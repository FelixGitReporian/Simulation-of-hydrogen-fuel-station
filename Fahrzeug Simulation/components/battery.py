...
class Battery(Serializable, Simulatable):
    ...
    def __init__(self, timestep, input_link,input_link1, file_path = None):
        '''
        Parameters
        ----------
        ...
        if self.specification == 'lithium_lfp_fuel':
            self.capacity_current_wh = self.capacity_nominal_wh
        elif self.specification == 'lithium_lfp_ele':
            self.capacity_current_wh = self.capacity_nominal_wh
    ...
    def calculate(self):
        ...
        self.acceleration = self.input_link1.acceleration
        ...
        # Charge case
            ...
            if (self.state_of_charge > self.charge_discharge_boundary):
                ...
                self.power_battery = round((self.power - ((abs(self.state_of_charge - self.charge_discharge_boundary)
                                    + self.power_self_discharge_rate) * self.capacity_current_wh / (self.timestep/3600))), 4)
    ...
    def battery_power(self):
        ...
        if self.power > 0. and self.acceleration[self.time] < 0: #charge (generator)
            self.efficiency =  self.charge_power_efficiency_a * (self.power/self.capacity_nominal_wh) + self.charge_power_efficiency_b
            self.recuperation_efficiency = 0.90 * 0.95 * 0.99 #Fahrzeug* Generator*Transmission
            self.power_battery = self.power * self.efficiency * self.recuperation_efficiency

        elif self.power > 0.: #charge (generator)
            self.efficiency =  self.charge_power_efficiency_a * (self.power/self.capacity_nominal_wh) + self.charge_power_efficiency_b
        ...
        elif self.power < 0.: #discharge (motor)
